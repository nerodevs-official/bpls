...
export default function MoIndex() {
    const { miscellaneous, filters, role } = usePage().props;
    const headers = miscellaneous?.headers ?? [];
    const tasks = miscellaneous?.tasks ?? [];
    const pagination = miscellaneous?.pagination ?? {};
    ...
    const getVal = (task, headerId) =>
        task?.values?.find((v) => v.miscellaneous_header_id === headerId)
            ?.value ?? "";
    ...
    const makeHref = (name, params) => {
        try {
            ...
            if (name === "miscellaneous.show")
                return `/miscellaneous/${params?.[0] ?? params}`;
            if (name === "miscellaneous.edit")
                return `/miscellaneous/${params?.[0] ?? params}/edit`;
            return "#";
        }
    };
    ...
    return (
        <AuthenticatedLayout
            ...
            // }
        >
            <Head title={miscellaneous?.name || "Request Letter"} />
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-600">
                            Manage and track all miscellaneous letters in one place.
                        </p>
                    </div>
                    <Link
                                                                    ...
                                                                    <td key={h.id} className="px-3 py-2">
                                                                        <button
                                                                            ...
                                                                            onClick={(e) => {
                                                                                ...
                                                                                if (!isFiled) {
                                                                                    router.put(
                                                                                        route("miscellaneous.update-status", {
                                                                                            id: task.id,
                                                                                        }),
                                                                                        {
                                                                                            status: "Filed",
