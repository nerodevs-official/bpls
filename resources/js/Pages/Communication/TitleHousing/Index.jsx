...
export default function MoIndex() {
    const { titlehousing, filters, role } = usePage().props;
    const headers = titlehousing?.headers ?? [];
    const tasks = titlehousing?.tasks ?? [];
    const pagination = titlehousing?.pagination ?? {};
    ...
    const H = {
        ...
        title: headerIdByName("TITLE"),
        status: headerIdByName("STATUS"),

        startdate: headerIdByName("START DATE"),
    ...
    const getVal = (task, headerId) =>
        task?.values?.find((v) => v.titlehousing_header_id === headerId)
            ?.value ?? "";
    ...
    }, [search, status, perPage, sortField, sortDirection]);
    ...
    }, [tasks, H.status, H.file, pagination.total]);
    ...
    const headerMeta = useMemo(() => {
        ...
        if (role === "Messenger") {
            keep = ["TITLE", "STATUS", "LINK/PDF FILE"];
        }
    ...
    const makeHref = (name, params) => {
        try {
            ...
            if (name === "titlehousing.show")
                return `/titlehousing/${params?.[0] ?? params}`;
            if (name === "titlehousing.edit")
                return `/titlehousing/${params?.[0] ?? params}/edit`;
            return "#";
        }
    };
    ...
    return (
        <AuthenticatedLayout
            ...
            // }
        >
            <Head title={titlehousing?.name || "Request Letter"} />
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-3xl uppercase font-bold text-gray-900">
                            title housing letter
                        </h1>
                        <p className="mt-1 text-sm text-gray-600">
                            Manage and track all title housing letters in one place.
                        </p>
                    </div>
                    <Link
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold text-gray-900">
                                    {Object.keys(stats.statusCounts).filter(k => stats.statusCounts[k] > 0).length}
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div className="flex-1">
                                <div className="relative">
                                    ...
                                    <input
                                        type="text"
                                        id="search"
                                        placeholder="Search reply letters..."
                                            ...
                                            return (
                                                ...
                                                >
                                                    {headerMeta.map((h) => {
                                                                    ...
                                                                    <td key={h.id} className="px-3 py-2">
                                                                        <button
                                                                            ...
                                                                            onClick={(e) => {
                                                                                ...
                                                                                if (!isFiled) {
                                                                                    router.put(
                                                                                        route("titlehousing.update-status", {
                                                                                            id: task.id,
                                                                                        }),
                                                                                        {
                                                                                            status: "Filed",
                                                                ...
                                                                <td key={h.id} className="px-3 py-2">
                                                                    {fileUrl && fileUrl.trim() !== "" ? (
                                                                        <button
                                                                            ...
                                                                            className="file-preview-btn inline-flex items-center gap-2 rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
                                                                        >
                                                        ...
                                                        if (h.name === "START DATE" || h.name === "DUE ON") {
                                                                ...
                                                                {openDropdownId === task.id && (
                                                                    ...
                                                                    >
                                                                        <div className="py-1">
                                                                            ...
                                                                            {(() => {
                                                                                const status = getVal(task, H.status);
                                        ...
                                        <tr>
                                            <td colSpan={headerMeta.length + 1} className="px-4 py-8 text-center">
                                                <div className="text-sm text-gray-500">
                                                    No reply letters found based on your permissions or filters.
                                                </div>
                                            </td>
                                        </tr>
                                    );
