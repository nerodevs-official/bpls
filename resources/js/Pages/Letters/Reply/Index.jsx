...
export default function MoIndex() {
    const { replyletter, filters, role } = usePage().props;
    const headers = replyletter?.headers ?? [];
    const tasks = replyletter?.tasks ?? [];
    const pagination = replyletter?.pagination ?? {};
    ...
    const getVal = (task, headerId) =>
        task?.values?.find((v) => v.replyletter_header_id === headerId)
            ?.value ?? "";
    ...
    const makeHref = (name, params) => {
        try {
            ...
            if (name === "replyletter.show")
                return `/replyletter/${params?.[0] ?? params}`;
            if (name === "replyletter.edit")
                return `/replyletter/${params?.[0] ?? params}/edit`;
            return "#";
        }
    };
    ...
    return (
        <AuthenticatedLayout
            ...
            // }
        >
            <Head title={replyletter?.name || "Reply Letter"} />
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-600">
                            Manage and track all reply letters in one place.
                        </p>
                    </div>
                    <Link
                                                                    ...
                                                                    <td key={h.id} className="px-3 py-2">
                                                                        <button
                                                                            ...
                                                                            onClick={(e) => {
                                                                                ...
                                                                                if (!isFiled) {
                                                                                    router.put(
                                                                                        route("replyletter.update-status", {
                                                                                            id: task.id,
                                                                                        }),
                                                                                        {
                                                                                            status: "Filed",
