...
export default function MoIndex() {
    const { requestletter, filters, role } = usePage().props;
    const headers = requestletter?.headers ?? [];
    const tasks = requestletter?.tasks ?? [];
    const pagination = requestletter?.pagination ?? {};
    ...
    const getVal = (task, headerId) =>
        task?.values?.find((v) => v.requestletter_header_id === headerId)
            ?.value ?? "";
    ...
    const makeHref = (name, params) => {
        try {
            ...
            if (name === "requestletter.show")
                return `/requestletter/${params?.[0] ?? params}`;
            if (name === "requestletter.edit")
                return `/requestletter/${params?.[0] ?? params}/edit`;
            return "#";
        }
    };
    ...
    return (
        <AuthenticatedLayout
            ...
            // }
        >
            <Head title={requestletter?.name || "Request Letter"} />
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-600">
                            Manage and track all request letters in one place.
                        </p>
                    </div>
                    <Link
                                                                    ...
                                                                    <td key={h.id} className="px-3 py-2">
                                                                        <button
                                                                            ...
                                                                            onClick={(e) => {
                                                                                ...
                                                                                if (!isFiled) {
                                                                                    router.put(
                                                                                        route("requestletter.update-status", {
                                                                                            id: task.id,
                                                                                        }),
                                                                                        {
                                                                                            status: "Filed",
