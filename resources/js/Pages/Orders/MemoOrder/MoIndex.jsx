...
export default function MoIndex() {
    const { order, filters, departments, role } = usePage().props;
    ...
    const getVal = (task, headerId) =>
        task?.values?.find((v) => v.order_header_id === headerId)?.value ?? "";
    ...
    const makeHref = (name, params) => {
        try {
            ...
            if (name === "memo-order.show")
                return `/memo-order/${params?.[0] ?? params}`;
            if (name === "memo-order.edit")
                return `/memo-order/${params?.[0] ?? params}/edit`;
            return "#";
        }
    };
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-600">
                            Manage and track all memo orders in one place.
                        </p>
                    </div>
                    <Link
                                                                    ...
                                                                    <td key={h.id} className="px-3 py-1">
                                                                        <button
                                                                            ...
                                                                            onClick={(e) => {
                                                                                ...
                                                                                if (!isFiled) {
                                                                                    router.put(
                                                                                        route("memo-order.update-status", {
                                                                                            id: task.id,
                                                                                        }),
                                                                                        {
                                                                                            status: "Filed",
                                        ...
                                        <tr>
                                            <td colSpan={headerMeta.length + 1} className="px-4 py-8 text-center">
                                                <div className="text-sm text-gray-500">
                                                    No memos found based on your permissions or filters.
                                                </div>
                                            </td>
                                        </tr>
                                    );
